...
const db = new sqlite3.Database('/Users/imseoghyeon/nodeDB');
...
export function createUser(req, res) {
  ...
  createHashPassword(password).then(({ hashedPassword, salt }) => {
    db.run('INSERT INTO users (name, email, password, salt, created_at) VALUES (?, ?, ?, ?, ?)', [name, email, hashedPassword, salt, createdAt],function (err) {
        if (err) {
